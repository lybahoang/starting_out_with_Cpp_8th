...
/***
 * Function removeByPosition: The function removes a node by its position.
 * If the position is negative or greater than or equal to the length of
 * the list, the function does nothing.
 */
void IntegerList::removeByPosition(int position)
{
    // If 'position' is less than 0 or greater than or equal to the length
    // of the list, then do nothing.
    if (position < 0 || position >= this->countNode())
        return;

    // Otherwise, search for the node indexed by 'position'.
    ListNode *nodePtr = nullptr;        // To traverse the list.
    ListNode *previousNode = nullptr;   // To point to the previous node.
    ...
    // A variable to keep track of the position.
    int positionCount = 0;

    // Skip all nodes whose position is less than 'position'.
    ...
    // Make the node after previousNode to be the node after nodePtr.
    previousNode->next = nodePtr->next;
